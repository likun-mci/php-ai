...
### Default persistence & dual-root storage

Since v1.63, the Agent supports **lazy automatic persistence**: you don't have to explicitly attach a `SessionManager` / `MemoryManager`. As soon as you signal intent to persist (set any one of `setSessionId()`, `setUserId()`, `setAgentHome()`, or `setProjectRoot()`), sessions and memory are automatically written to disk.

```php
$agent
    ->setWorkdir('/var/www/project')
    ->setUserId('u_10086')          // hard requirement for multi-tenant: isolate per user
    ->setSessionId(Agent::newSessionId());

$agent->chat('Remember I prefer dark theme');
// A fresh instance resumes as long as userId + sessionId match
```

> **Behavior change (upgrade note)**: from this version on, merely calling `setSessionId()` may produce disk data. If you want pure in-memory behavior, turn it off explicitly:
>
> ```php
> $agent->autoPersist(false);
> ```

**Dual-root storage**: data lands in two places with different semantics —

| Root | Location | Contents | Mode |
|------|----------|----------|------|
| Project root | `<project>/.agent/AGENT.md` | Long-term project knowledge valuable to the team/other Agents (can be committed to Git) | `0755` |
| HOME root | `~/.agent/` (`users/`, `projects/`, `sessions/` …) | User input/output, absolute paths, session IDs, etc. | `0700` |

The project root is found by walking up from `workdir` looking for `.agent/` (stops at HOME, max 10 levels, does not rely on `.git`, does not use `realpath()`); if not found, `workdir` is used. For cross-directory deployments (blue-green releases, CI vs dev-machine paths differing) that need session reuse, pin the project identity explicitly with `setProjectRoot()`.

**Multi-tenant isolation**: after `setUserId()`, user memory and sessions are sharded per user, and the raw `userId` **never appears in the disk path** (only its SHA-256 shard is stored). Sessions are ownership-checked on load: loading is refused if the current `userId` / project identity doesn't match the session record. `sessionId` is generated by `Agent::newSessionId()` (128-bit random), but it is **not an authorization credential** — the application layer must still verify the user owns the target session.

**JSONL session storage**: auto-mounting uses `JsonlSessionStore` — one `<sid>.jsonl` per session (one event per line) plus `<sid>.state.json` (non-message state). Messages usually only grow, so appending a line costs O(new); only compact / rollback triggers a full rewrite (temp file + atomic rename). The whole read-modify-write runs under a `<sid>.lock` exclusive lock to avoid concurrent lost updates. Existing users of `new FileSessionStore(...)` are unaffected; old `<sid>.json` files remain readable and are not auto-migrated. To convert legacy sessions, run `php bin/agent-migrate-sessions.php <dir>`.

**Memory vs. Instruction** — the two are physically and semantically separate; don't conflate them:

| File | Nature |
|------|--------|
| `AGENTS.md` / `CLAUDE.md` | Human-written rules/procedures — read-only **Instruction** for the Agent |
| `.agent/AGENT.md` | **Memory** the Agent maintains automatically and reuses across sessions |

**Git**: the library **never** touches your `.gitignore` / `git config` / `git status`. Whether to commit the project memory `.agent/AGENT.md` is entirely up to your project; if you do commit it, you can configure your own `.agent/.gitignore`.
...
#### Model-driven memory (remember / forget tools)

Since v1.65, `remember()` prefixes each entry with a **date + content-hash id**:

```text
- [2026-09-03] (#9bda3a) The project uses CodeIgniter 3
```

The id is a content hash (not random/incrementing), so **repeating the same `remember()` is naturally idempotent** and won't bloat the memory file. `remember($scope, $content, ['date' => false])` disables the date prefix (the id is still written). Deletion targets a precise id:

```php
$mm->remember('project', 'The project uses CodeIgniter 3');  // writes an entry with an id
$mm->forgetById('project', '9bda3a');                        // delete by exact id
$mm->findByPattern('project', 'CodeIgniter');                // find candidates by substring (no delete)
```

When memory is attached and the Agent is a **tool-using Agent** (e.g. `codeAgent()`), the library auto-registers the `remember` / `forget` tools so the model can record/delete conclusions itself. Memory writes are a persistent side effect and are gated by `PermissionManager` by default (`dont_ask` / `bypass` allow through). On injection, long-term scopes (`agent` / `user` / `project`) are **resident** while short-term scopes (`session` / `task`) are relevance-filtered by goal — so long-term memory never vanishes even when a given retrieval has zero hits.

**Auto-consolidation** is off by default — automatically distilling long-term memory from every result is far riskier than auto-saving a session, so it must be opted into explicitly:

```php
$agent->autoConsolidate(true);   // after each run()/chat(), distill candidates from the result, dedupe, then write
```
