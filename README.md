...
Text::cutBytes($output, 1024);      // 限字节，不切开字符（工具输出、diff 用这个）
...
1. 最后一条 assistant 消息带完成标记（「任务完成」「已解决」「all done」…）→ 完成
2. **最后一批**工具结果里还有报错 → 继续修；同一个报错连撞两批则停手，继续重试没有进展
3. 一个工具都没调过、且还在首轮 → 继续（模型还停在对话阶段）
4. 其余 → 完成
...
`maxRounds` 是兜底：**反思次数**达到上限后强制结束，避免模型在「还差一点」的判断里空转。它数的是反思做过几轮（`reflection_round`），不是 Agent 循环的迭代号（`iteration`）——两者语义不同，混用会变成「跑到第 N 轮迭代之后反思一律放行」，哪怕工具还在报错。

这道闸排在判据**之后**：自定义策略永远会被调用，只是它说「还要继续」而轮次已到顶时，由上限收口。（排在前面会让注入的策略在轮次超限后根本不执行，排查时只会怀疑自己的策略写错了。）
...
`codeAgent()` 会装上一份默认系统提示词（`CodeAgentPrompt`），告诉模型这些工具**什么时候**该用——工具描述只说明「这个工具做什么」，回答不了「什么时候该用它」。

自己调过 `setSystem()` 就不会被覆盖：

```php
$agent = Agent::create($ai)->workdir($dir)->codeAgent();          // 用默认提示词
$agent = Agent::create($ai)->setSystem('...')->codeAgent();       // 用你自己的
echo \Ai\Agent\CodeAgentPrompt::build(['test' => 'composer test']);  // 看看默认写了什么
```

它讲四件事：动手前先读懂现状；改**已有**文件一律用 `edit_file` 增量改（`write_file` 只用于新建）；三件以上的任务先用 `update_plan` 写步骤；改完必须实际验证。
...
| `update_plan` 调用 | 0 次 | 3 次 | 2 次 |
| 改已有文件的方式 | `write_file` 整体重写 ×3 | `edit_file` ×6 | `edit_file` ×5（并列发出） |
| 测试结果 | ALL PASS | ALL PASS | ALL PASS |

三次产出都正确，差别在**过程安全性与成本**。无提示词那次用 `write_file` 整体重写了三个已有文件——这次文件小、重写对了，但在几百行的文件上，整体重写正是静默丢代码的经典路径。默认提示词用大约 60% 的成本增量换掉这个风险；补上「想清楚的改动一次全部发出」之后，增量降到约 40%。
